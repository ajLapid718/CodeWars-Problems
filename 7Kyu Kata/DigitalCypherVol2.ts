...
export function decode (code: Array<number>, n: number): String {
  let message: String = "";
  let key: String = n.toString();

  for (let i = 0; i < code.length; i++) {
    let currNum = code[i];
    let decryptedNum = currNum - Number(key[i % key.length]);
    let decryptedLetter = String.fromCharCode(96 + decryptedNum)
