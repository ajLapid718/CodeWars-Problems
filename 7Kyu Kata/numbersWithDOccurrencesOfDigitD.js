...
function isDd(num) {
  ...
  for (var i = 0; i < stringifiedNum.length; i++) {
    dictionary[parseInt(stringifiedNum[i])] = dictionary[parseInt(stringifiedNum[i])] ? dictionary[parseInt(stringifiedNum[i])] + 1 : 1;
  }

  for (var j = 0; j < stringifiedNum.length; j++) {
    if (parseInt(stringifiedNum[j]) === dictionary[stringifiedNum[j]]) {
      return true;
    }
  }
