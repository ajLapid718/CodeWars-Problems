...
function authList(arr) {
  var usernamesCount = arr.length;
  var usernames = arr.join(' ');
  var properUsernames = usernames.match(/(?=.*[a-z])(?=.*[0-9])[a-z0-9]{6,10}/g);

  if (properUsernames === null) {
    return false;
  }

  return usernamesCount === properUsernames.length;
}
