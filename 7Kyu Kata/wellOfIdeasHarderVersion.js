...
function well(x) {
  var amountOfGoodIdeas = 0;

  for (var i = 0; i < x.length; i++) {
    var currentArray = x[i];

    for (var j = 0; j < currentArray.length; j++) {
      var currentIdea = currentArray[j];

      if (typeof currentIdea !== 'string') {
        continue;
      }

      if (currentIdea.toLowerCase() === 'good') {
        amountOfGoodIdeas += 1;
      }
    }
  }

  if (amountOfGoodIdeas > 2) {
    return 'I smell a series!';
  } else if (amountOfGoodIdeas > 0) {
    return 'Publish!';
  } else {
    return 'Fail!';
  }
