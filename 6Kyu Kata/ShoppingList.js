...
function shoppingListCost(arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    var nameOfFood = currentItem[0];
    var quantityOfFood = currentItem[1];

    var price = groceries[nameOfFood].price;
    var discount = groceries[nameOfFood].discount;
    var bogof = groceries[nameOfFood].bogof;

    if (bogof === true) {
      quantityOfFood = quantityOfFood / 2;
    }

    var subTotal = price * Math.round(quantityOfFood);

    totalPrice = totalPrice + subTotal - (subTotal * discount/100);
  }
