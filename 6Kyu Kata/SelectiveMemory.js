...
=> We must remove every memories of Sarah Connor because she's marked, but as a side-effect we must also remove all the memories about Marilyn Monroe that comes right after her!
...
function select(memory) {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    if (currentName[0] === '!') {
      let nameToForget = currentName.slice(1);
      let otherNameToForget = names[i+1];
      forgottenNames[nameToForget] = true;
      forgottenNames[otherNameToForget] = true;
      continue;
    }

    if (forgottenNames.hasOwnProperty(currentName)) {
      continue;
    }
  ...
  while (i < res.length) {
    if (forgottenNames.hasOwnProperty(res[i])) res.splice(i, 1);
    i++;
  }
