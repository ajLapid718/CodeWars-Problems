...
If there is no winner, return null (in Java and JavaScript), None (in Python) or nil (in Ruby).

Task Description
There are no given candidates.
An elector can vote for anyone.
...
function getWinner(listOfBallots) {
  let obj = {};
  let majorityVote = Math.floor(listOfBallots.length/2);

  for (let i = 0; i < listOfBallots.length; i++) {
    let candidate = listOfBallots[i];
    obj[candidate] = obj[candidate] + 1 || 1;
  }

  for (let key in obj) {
    let val = obj[key];
    if (val > majorityVote) return key;
  }
...
function getWinner(listOfBallots) {
  ...
  for (let i = 0; i < listOfBallots.length; i++) {
    let candidate = listOfBallots[i];
    obj[candidate] = obj[candidate] + 1 || 1;
    if (obj[candidate] > majorityVote) return candidate;
  }
