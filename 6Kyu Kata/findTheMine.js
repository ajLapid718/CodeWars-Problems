...
The location returned should be an array (Tuple<int, int> in C#) where the first element is the row index, and the second element is the column index of the bomb location (both should be 0 based).
...
function mineLocation(field) {
  ...
  for (let i = 0; i < field.length; i++) {
    ...
    locations = locations.concat(currentArr);
  }

  for (let j = 0; j < locations.length; j++) {
    let currentNum = locations[j];
    if (currentNum === 1) {
