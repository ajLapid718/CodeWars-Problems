...
function meeting(x, need) {
  ...
  for (let i = 0; i < x.length; i++) {
    let room = x[i];
    let amountOfOccupants = room[0].length;
    let amountOfChairs = room[1];

    if (amountOfChairs <= amountOfOccupants) {
      arr.push(0);
      continue;
    }

    let amountOfSpareChairs = amountOfChairs - amountOfOccupants;
    let chairsLeftToGrab = need - tracker;

    if (amountOfSpareChairs >= chairsLeftToGrab) {
      arr.push(chairsLeftToGrab);
      return arr;
    }
    else {
      arr.push(amountOfSpareChairs);
      tracker += amountOfSpareChairs;
      if (tracker === need) return arr;
    }
  }
