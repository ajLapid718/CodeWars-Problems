...
But.. didn't that look like a lot of JavaScript with some HTML mixed in?
It makes it kinda impossible for someone who only knows HTML to modify templates and can make the code kinda tough to read.
Fortunately for us, React works with JSX

JSX essentially extends normal JavaScript syntax to allow us to create XML nodes directly in our JavaScript files.
This makes it super easy to create React elements on the fly! Check this hello world out:

var helloWorld = <div>Hello World</div>
Clean, huh? Let's also experiment with the React Components.
React gives us access to "components", which can be leveraged to modularize reusable functionality that will render out HTML templates.
...
var dan = <Person name={'dan'}></Person>
...
Oh, and finally, don't forget to add a key property to each codewarrior's anchor tag.
It should be unique for each codewarrior, so choose a key wisely! =)

*/

const React = require('react');

const myHeader = <h1>My three favorite codewarriors are Anna, Brian and Chris</h1>
...
const Leaderboard = React.createClass({
  render: function() {
    return (
      <div>
        {this.props.leaders.map(leader => <CodewarsLink key={leader} user={leader} ></CodewarsLink>)}
      </div>
    )
  }
});
