...
If this is not possible, parts 1 to 5 will have subsequently non-increasing lengths, such that parts 1 to 4 are at least as long as when evenly divided, but at most 1 longer.
...
function movingShift(str, key) {
  let len = str.length;
  let firstFourRunnersLen = Math.ceil(len / 5);
  let lastRunnerLen = len - (firstFourRunnersLen * 4);
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (counter === firstFourRunnersLen - 1) {
      tempStr += character;
      arr.push(tempStr);
      tempStr = "";
      counter = 0;
    }
    else if (i >= firstFourRunnersLen * 4) {
      tempStr += character;
    }
    else {
      tempStr += character;
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 0; j < currStr.length; j++) {
      ...
      if (isLowerCase(currLetter) === true) {
        if (shiftedLetterAsciiVal > 122) {
          shiftedLetterAsciiVal = (shiftedLetterAsciiVal % 122) + 96;
        }
      }

      if (isUpperCase(currLetter) === true) {
        if (shiftedLetterAsciiVal > 90) {
          shiftedLetterAsciiVal = (shiftedLetterAsciiVal % 90) + 64;
        }
      }

      temp += String.fromCharCode(shiftedLetterAsciiVal);
      key++;
    }
...
function demovingShift(arr, key) {
  let len = arr.map(str => str.length).reduce((acc, curr) => acc + curr);
  ...
  for (let i = 0; i < tempArr.length; i++) {
    let character = tempArr[i];

    if (!isLowerCase(character) && !isUpperCase(character)) {
      tempStr += character;
      key++;
      continue;
    }

    let asciiValOfLetter = character.charCodeAt();
    let shiftedLetterAsciiVal = asciiValOfLetter - (key % 26);

    if (isLowerCase(character) === true) {
      if (shiftedLetterAsciiVal < 97) {
        shiftedLetterAsciiVal = (shiftedLetterAsciiVal % 71) + 97;
      }
    }

    if (isUpperCase(character) === true) {
      if (shiftedLetterAsciiVal < 65) {
        shiftedLetterAsciiVal = (shiftedLetterAsciiVal % 39) + 65;
      }
    }

    tempStr += String.fromCharCode(shiftedLetterAsciiVal);
    key++;
  }
...
// HELPER FUNCTIONS TO DETERMINE LETTER CASE STATUS;
